...
async function main() {
  ...
  if (
    !sessionStorage.getItem("v-session-id") ||
    window.location.pathname !== sessionStorage.getItem("bounded-video")
  ) {
    const uniqueVideoId =
...
async function setupPlayer(groupVideo) {
  ...
  player.addEventListener("rate-change", (e) =>
    track("Speed", `Change to ${e.detail}`)
  );
  ...
  $(document).ready(() => {
    ...
    $(".vds-blocker").hide();
    $("media-play-button.plyr__control--overlaid").css(
      "pointer-events",
      "none"
    );
  });
...
function onVideoStart(player) {
  ...
  $('.plyr__control[data-plyr="speed"][value="1"]').css({
  ...
  $('.plyr__control[data-plyr="speed"]').on("click", function (e) {
    ...
    $('.plyr__control[data-plyr="speed"]').css({
      "background-color": "",
      color: "#495463",
    });
...
function checkQuiz() {
  var check_quiz_interval = window.setInterval(function () {
    ...
    for (var i = 0; i < data.items.length; i++) {
      switch (data.items[i].type) {
        case "text":
          var $li = $("<li></li>");
          var div =
            "<h3> <i class='fa fa-question-circle-o' aria-hidden='true'></i> Question " +
            (parseInt(i) + 1) +
            " </h3>" +
            "<h4><strong>" +
            data.items[i].title +
            "</strong></h4>" +
            "<textarea name='' id='quiz_q" +
            (parseInt(i) + 1) +
            "' cols='20' rows='3' placeholder='Please input your answer'></textarea>";
          $li.append(div);
        ...
        case "multiple_choice":
          ...
          for (var j = 0; j < data.items[i].list.length; j++) {
            var choice =
              "<li class='list__item'>" +
              "<input type='radio' class='radio-btn' name='choise_" +
              (parseInt(i) + 1) +
              "' id='choise_" +
              (parseInt(i) + 1) +
              "_" +
    ...
    $("#submit_result").on("click", function () {
      ...
      for (var i = 0; i < data.items.length; i++) {
        switch (data.items[i].type) {
          ...
          case "multiple_choice":
            var user_ans = $(
              ".client_question_list_wrap input[name='" +
                "choise_" +
                (parseInt(i) + 1) +
                "']:checked"
            )
          ...
          switch (feedback_array[i][item]) {
            case true:
              var th =
                ...
                "<img src='../../img/tick.png' alt='' style='width:32px; height:auto;'>" +
                "</td>";
              break;
            case false:
              var th =
                "<td style='text-align:center;'>" +
                "<img src='../../img/cancel.png' alt='' style='width:32px; height:auto;'>" +
                "</td>";
              break;
            default:
              break;
